...
for (DGP in c("linear","quad","quad_long")) {
  for (homoscedastic in c(T,F)) {
    for (accuracy in c("medium")) {
      for (R21 in c(0, 0.2, 0.4, 0.6, 0.8)) {
        for (R22 in c(0, 0.2, 0.4, 0.6, 0.8)) {
          for (i in 1:replications) {
            ...
            eta0 = beta0 = rep(0,p)
            if (DGP == "linear") {
              beta0[1:5] = beta0[11:15] = (1/(1:5))
              eta0[1:10] = (1/(1:10))
            } else if  (DGP == "quad") {
              beta0[1:5] = beta0[11:15] = (1/(1:5))^2
              eta0[1:10] = (1/(1:10))^2
            } else if (DGP == "quad_long") {
              beta0[1:5] =  (1/(1:5))^2
              beta0[11:200] = (1/(1:190))^2
              eta0[1:10] = (1/(1:200))^2
            }
            ...
            print(paste("R21: ",R21, " - R22: ",R22, " - Accuracy: ", accuracy, " - Iteration: ",i, sep ="")) 
          }
          ...
          # 
          # output_mat = data.frame(matrix(ncol = 5, nrow = 2, dimnames=list(NULL, c("binom", "unif", "adaptive", "score","control"))), 
          #                         row.names = c("bias","coverage"))
          # output_mat[1,] = sqrt(colMeans((all_bias)^2, na.rm = T))
          # output_mat[2,] = colMeans(all_coverage, na.rm = T)
          ...
          # dgp, homo, accuracy, r21, r22
          
          output_name = paste("output/",DGP,"_",error_name,"_",R21,"_",R22,"_",accuracy,".csv",sep = "")
